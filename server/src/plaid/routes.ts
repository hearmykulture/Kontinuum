  ...
  if (!svcPromise) {
    svcPromise = (async () => {
      ...
      const svc: Partial<PlaidService> = {
        ...
        createSandboxItem: mod.createSandboxItem ?? mod.default?.createSandboxItem,
        transactionsRefresh: mod.transactionsRefresh ?? mod.default?.transactionsRefresh,
...
plaidRouter.post('/sandbox/create-item', async (req, res, next) => {
  try {
    ...
    const out = await createSandboxItem({ userId, institutionId, institutionName });
    res.json(out);
  } catch (e) {
    next(e);
  }
...
function toWebhookCodeEnum(input?: string): SandboxItemFireWebhookRequestWebhookCodeEnum {
...
plaidRouter.post('/sandbox/fire-webhook', async (req, res, next) => {
  try {
    ...
    if (!secret?.accessToken) return res.status(404).json({ error: 'No Plaid secret for itemId' });
    ...
    if (status) return res.status(500).json({ error: 'Internal Server Error', ...err });
    next(e);
  }
});
...
<body style="font-family: system-ui; display:grid; place-items:center; height:100vh; background:#0B2B26; color:#DAF1DE;">
  <div>
    ...
    <script>setTimeout(function(){ window.location = ${JSON.stringify(deeplink)}; }, 200);</script>
  </div>
</body></html>`;
